...
def generate_images(output_folder, num_images, num_squares):
    ...
    for i in range(num_images):
        ...
        for _ in range(num_squares):
            ...
            side_length = random.randint(12, 20)  # Range for the square's side length
            x = random.randint(0, quarter_width)
            y = random.randint(0, quarter_height)

            # Draw a white filled square in the bottom-left part relative to the diagonal in the top-left quadrant
            draw.rectangle([(x - side_length, y - side_length), (x + side_length, y + side_length)], fill="white")

            # Mirror to generate the top-right white filled square
            draw.rectangle([(y - side_length, x - side_length), (y + side_length, x + side_length)], fill="white")
            ...
            draw.rectangle([(y - side_length, 2 * quarter_height - x - side_length - 0.5),
                            (y + side_length, 2 * quarter_height - x + side_length - 0.5)], fill="white")
            ...
            draw.rectangle([(2 * quarter_width - y - side_length - 0.5, x - side_length),
                            (2 * quarter_width - y + side_length - 0.5, x + side_length)], fill="white")
            ...
            draw.rectangle([(2 * quarter_width - y - side_length - 0.5, 2 * quarter_height - x - side_length - 0.5),
                            (2 * quarter_width - y + side_length - 0.5, 2 * quarter_height - x + side_length - 0.5)], fill="white")
        # Save the image
